...
Status: Самая актуальная версия на 21 октября 2025
...
<git_rules>
1) always use origin that claimed in README.md file in root folder
2) use main as the main branch
3) Make atomic commits by logical groups. The commit message language — should relate to most files in commit.
4) Keep commits atomic: commit only the files you touched and list each path explicitly. For tracked files run `git commit -m "<scoped message>" -- path/to/file1 path/to/file2`
5) For brand-new files, use the one-liner `git restore --staged :/ && git add "path/to/file1" "path/to/file2" && git commit -m "<scoped message>" -- path/to/file1 path/to/file2`
</git_rules>
...
* In case you have significant piece of important factual information, then put it in a FACTS.md in relevant project folder
    ...
    8. Source Every Statement: Use [CANONICAL] for first mentions and [REF: path#section] elsewhere (see [REF: AGENTS_V2.md#tag-glossary])
